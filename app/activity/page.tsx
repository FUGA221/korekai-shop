...
const activities = [
  {
    ...
    summary:
      "長野県千曲市の工房アプリコさんを訪問。あんずジャムの製造工程を見学し、地域の魅力を改めて実感しました。",
    tag: "現地訪問",
  ...
  },
  {
    ...
    summary:
      "プロジェクトの紹介やパートナー情報を発信するWebサイトを公開。今後も活動の様子をお届けしていきます。",
    tag: "お知らせ",
  ...
  },
  {
    ...
    summary:
      "メンバー全員で今後の方針を共有。地域の生産者との連携方法やコンサルティングの進め方について議論しました。",
    tag: "ミーティング",
        ...
        <div className="max-w-3xl mx-auto px-6">
          <div className="space-y-6">
            {activities.map((item) => (
              <article
                key={item.id}
                className="card-hover bg-white/60 backdrop-blur-sm rounded-2xl p-6 md:p-8 border border-warmBrown/8"
              >
                <div className="flex flex-wrap items-center gap-3 mb-3">
                  <time className="text-xs font-sans text-warmGray">
                    {item.date}
                  </time>
                  <span
                    className={`text-xs font-sans font-medium px-3 py-1 rounded-full ${item.tagColor}`}
                  >
                    {item.tag}
                  </span>
                </div>
                <h2 className="font-serif text-lg md:text-xl font-bold text-darkBrown mb-3">
                  {item.title}
                </h2>
          ...
          <div className="mt-12 text-center bg-lightSand/60 rounded-2xl p-8 border border-warmBrown/8">
            <p className="font-serif text-lg font-bold text-darkBrown mb-2">
              最新情報はInstagramでも発信中
            </p>
            <p className="font-sans text-sm text-warmBrown mb-5">
              日々の活動やイベントの様子をリアルタイムでお届けしています。
            </p>
          ...
          <div className="mt-8 text-center">
            <p className="font-sans text-xs text-warmGray">
              ※ 掲載内容はサンプルです。実際の活動に合わせて随時更新されます。
            </p>
          </div>
        </div>
      </section>
